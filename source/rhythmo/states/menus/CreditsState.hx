...
class CreditsState extends ExtendableState
{
	...
	override public function update(elapsed:Float):Void
	{
		...
		if (Input.justPressed('accept') && credData.users[curSelected].urlData[curSocial][1] != null)
		{
			if ((credData.users[curSelected].urlData[curSocial][1] == 'nolink'))
				...
				Util.openUrlPlease(credData.users[curSelected].urlData[curSocial][1]);
		}
	...
	private function updateSectionName():Void
	{
	...
	private function updateSocial(huh:Int = 0):Void
	{
	...
	private function updateBottomMarker():Void
	{
...
class CreditsIcon extends GameSprite
{
	...
	public function new(x:Float = 0, y:Float = 0, icon:String = ''):Void
	{
		super(x, y);
