...
class PlayState extends ExtendableState
{
	...
	override public function update(elapsed:Float):Void
	{
		...
		accuracy = (!SaveData.settings.botPlay) ? Util.boundTo(Math.floor((score * 100) / ((hits + misses) * 3.5)) * 0.01, 0, 100) : 0;
		...
		for (note in notes)
		{
			var strum = strumline.members[Util.getNoteIndex(note.dir)];
	...
	function inputFunction():Void
	{
		...
		if (possibleNotes.length > 0)
		{
			for (i in 0...possibleNotes.length)
			{
				...
				if ((justPressed[Util.getNoteIndex(note.dir)]
					&& !doNotHit[Util.getNoteIndex(note.dir)]
					...
					|| SaveData.settings.botPlay)
				{
					...
					noteDataTimes[Util.getNoteIndex(note.dir)] = note.strum;
					doNotHit[Util.getNoteIndex(note.dir)] = true;

					if (!SaveData.settings.botPlay)
						strumline.members[Util.getNoteIndex(note.dir)].press();
			...
			if (possibleNotes.length > 0)
			{
				for (i in 0...possibleNotes.length)
				{
					...
					if (note.strum == noteDataTimes[Util.getNoteIndex(note.dir)] && doNotHit[Util.getNoteIndex(note.dir)])
						destroyNote(note);
				}
			}
		}
	...
	function noteHit(note:Note, rating:String):Void
	{
		...
		if (rating == 'perfect' || rating == 'perfect-golden')
		{
			...
			splash.setupSplash(note.x, note.y, Util.getNoteIndex(note.dir));
